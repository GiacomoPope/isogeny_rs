mod benchmark_sqisign_one {
    use criterion::{Criterion, black_box, criterion_group};
    use isogeny::protocols::sqisign_parameters::SQISIGN_I;
    use std::time::Duration;

    static MSG: &str = "D81C4D8D734FCBFBEADE3D3F8A039FAA2A2C9957E835AD55B22E75BF57BB556AC8";
    static PK: &str = "07CCD21425136F6E865E497D2D4D208F0054AD81372066E817480787AAF7B2029550C89E892D618CE3230F23510BFBE68FCCDDAEA51DB1436B462ADFAF008A010B";
    static SIG: &str = "";

    fn benchmark_sqisign_i_verification(c: &mut Criterion) {
        let msg: &[u8] = &hex::decode(MSG).unwrap();
        let pk_bytes: &[u8] = &hex::decode(PK).unwrap();
        let sig_bytes: &[u8] = &hex::decode(SIG).unwrap();
        assert!(SQISIGN_I.verify(msg, sig_bytes, pk_bytes));

        let bench_id = format!("Benchmarking SQIsign I Verification",);
        c.bench_function(&bench_id, |b| {
            b.iter(|| {
                SQISIGN_I.verify(black_box(&msg), black_box(&sig_bytes), black_box(&pk_bytes))
            })
        });
    }

    criterion_group! {
        name = sqisign_benchmarks;
        config = Criterion::default().measurement_time(Duration::from_secs(10));
        targets = benchmark_sqisign_i_verification
    }
}

mod benchmark_sqisign_three {
    use criterion::{Criterion, black_box, criterion_group};
    use isogeny::protocols::sqisign_parameters::SQISIGN_III;
    use std::time::Duration;

    static MSG: &str = "D81C4D8D734FCBFBEADE3D3F8A039FAA2A2C9957E835AD55B22E75BF57BB556AC8";
    static PK: &str = "C32377D6F6D70729884A7F6877EF4791E35D21F751A3E96DE23F9A7A3C01BCD8A5F146DC19E4E2AC63007457F97D8A40EE84AEE7564CA9A7FBE6200FD3E5E55901BFC60EB25C50D39F5C91C96510556BAA22028DF76360841721A601D65E8D0F06";
    static SIG: &str = "";

    fn benchmark_sqisign_iii_verification(c: &mut Criterion) {
        let msg: &[u8] = &hex::decode(MSG).unwrap();
        let pk_bytes: &[u8] = &hex::decode(PK).unwrap();
        let sig_bytes: &[u8] = &hex::decode(SIG).unwrap();
        assert!(SQISIGN_III.verify(msg, sig_bytes, pk_bytes));

        let bench_id = format!("Benchmarking SQIsign III Verification",);
        c.bench_function(&bench_id, |b| {
            b.iter(|| {
                SQISIGN_III.verify(black_box(&msg), black_box(&sig_bytes), black_box(&pk_bytes))
            })
        });
    }

    criterion_group! {
        name = sqisign_benchmarks;
        config = Criterion::default().measurement_time(Duration::from_secs(10));
        targets = benchmark_sqisign_iii_verification
    }
}

mod benchmark_sqisign_five {
    use criterion::{Criterion, black_box, criterion_group};
    use isogeny::protocols::sqisign_parameters::SQISIGN_V;
    use std::time::Duration;

    static MSG: &str = "D81C4D8D734FCBFBEADE3D3F8A039FAA2A2C9957E835AD55B22E75BF57BB556AC8";
    static PK: &str = "";
    static SIG: &str = "";

    fn benchmark_sqisign_v_verification(c: &mut Criterion) {
        let msg: &[u8] = &hex::decode(MSG).unwrap();
        let pk_bytes: &[u8] = &hex::decode(PK).unwrap();
        let sig_bytes: &[u8] = &hex::decode(SIG).unwrap();
        assert!(SQISIGN_V.verify(msg, sig_bytes, pk_bytes));

        let bench_id = format!("Benchmarking SQIsign V Verification",);
        c.bench_function(&bench_id, |b| {
            b.iter(|| {
                SQISIGN_V.verify(black_box(&msg), black_box(&sig_bytes), black_box(&pk_bytes))
            })
        });
    }

    criterion_group! {
        name = sqisign_benchmarks;
        config = Criterion::default().measurement_time(Duration::from_secs(10));
        targets = benchmark_sqisign_v_verification
    }
}

fn main() {
    benchmark_sqisign_one::sqisign_benchmarks();
    benchmark_sqisign_three::sqisign_benchmarks();
    benchmark_sqisign_five::sqisign_benchmarks();
}
