...
#[inline]
fn curve_from_A_plus_minus<Fq: FqTrait>(A24_plus: &Fq, A24_minus: &Fq) -> Curve<Fq> {
...
/// Cost: 5S + 7M
#[inline(always)]
fn xtpl_proj<Fq: FqTrait>(A24_plus: &Fq, A24_minus: &Fq, P: &mut PointX<Fq>) {
    let mut t0 = P.X - P.Z;
...
/// Cost: n * (5S + 7M)
fn xtpl_proj_iter<Fq: FqTrait>(A24_plus: &Fq, A24_minus: &Fq, P: &mut PointX<Fq>, n: usize) {
    for _ in 0..n {
        xtpl_proj(A24_plus, A24_minus, P);
    }
}
...
/// Cost: 3S + 2M
#[inline]
fn three_isogeny_codomain<Fq: FqTrait>(P: &PointX<Fq>) -> (Fq, Fq, Fq, Fq) {
    let c0 = P.X - P.Z;
    let t0 = c0.square();
    let c1 = P.X + P.Z;
    let t1 = c1.square();
...
/// Cost: 2S + 4M
#[inline(always)]
fn three_isogeny_eval<Fq: FqTrait>(c0: &Fq, c1: &Fq, Q: &mut PointX<Fq>) {
    let mut t0 = Q.X + Q.Z;
    let mut t1 = Q.X - Q.Z;
    t0 *= *c0;
    t1 *= *c1;
...
pub fn three_isogeny_chain_naive<Fq: FqTrait>(
    domain: &Curve<Fq>,
    kernel: &PointX<Fq>,
    n: usize,
    ...
    let mut A24_plus = domain.A + Fq::TWO;
    let mut A24_minus = domain.A - Fq::TWO;
    ...
    for i in 0..n {
        ...
        ker_step = ker;
        xtpl_proj_iter(&A24_plus, &A24_minus, &mut ker_step, n - i - 1);

        // Compute the codomain from ker_step
        (A24_plus, A24_minus, c0, c1) = three_isogeny_codomain(&ker_step);

        // Push through the kernel
        three_isogeny_eval(&c0, &c1, &mut ker);

        // Push through the points to evaluate
        for P in images.iter_mut() {
            three_isogeny_eval(&c0, &c1, P);
        }
    }
    curve_from_A_plus_minus(&A24_plus, &A24_minus)
}

/// Compute a 3^n isogeny using a balanced strategy
pub fn three_isogeny_chain<Fq: FqTrait>(
    domain: &Curve<Fq>,
    kernel: &PointX<Fq>,
    n: usize,
    images: &mut [PointX<Fq>],
    ...
    for _ in 0..n {
        ...
        while orders[k] != 1 {
            ...
            xtpl_proj_iter(&A24_plus, &A24_minus, &mut stategy_points[k], m);
        ...
        for i in 0..space {
            three_isogeny_eval(&c0, &c1, &mut stategy_points[i]);
