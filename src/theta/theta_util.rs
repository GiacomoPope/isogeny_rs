...
/// Cost: 4S
#[inline(always)]
pub fn to_square<Fq: FqTrait>(X: &Fq, Y: &Fq, Z: &Fq, T: &Fq) -> (Fq, Fq, Fq, Fq) {
    let XX = X.square();
    let YY = Y.square();
    let ZZ = Z.square();
    let TT = T.square();
...
/// Cost: 4S, 8a
#[inline(always)]
pub fn to_hadamard_square<Fq: FqTrait>(X: &Fq, Y: &Fq, Z: &Fq, T: &Fq) -> (Fq, Fq, Fq, Fq) {
    let (XX, YY, ZZ, TT) = to_square(X, Y, Z, T);
