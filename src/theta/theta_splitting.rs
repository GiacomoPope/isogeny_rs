...
impl<Fq: FqTrait> ThetaStructure<Fq> {
    ...
    fn set_cond_matrix(M1: &mut [Fq], M2: &[Fq], ctl: u32) {
        M1.iter_mut()
            .zip(M2.iter())
            .for_each(|(a, b)| a.set_cond(b, ctl));
    }

    const SPLITTING_MAPS: [[Fq; 16]; 10] = [
        [
            <Fq>::ONE,
            <Fq>::ZERO,
            <Fq>::ZERO,
    ...
    /// `0x00..00` failure.
    fn compute_splitting_matrix(null_point: &ThetaPoint<Fq>) -> ([Fq; 16], u32) {
        ...
        let mut t2: Fq;
        let mut U_sqr: Fq;
        let null_coords: [Fq; 4] = null_point.into();
        ...
        for i in 0..10 {
            ...
            count += ctl & 1;
            Self::set_cond_matrix(&mut M, &Self::SPLITTING_MAPS[i], ctl);
        }
    ...
    pub fn splitting_isomorphism(
        ...
        let mut O0 = self.null_point();
        let (M, ok) = Self::compute_splitting_matrix(&O0);
...
impl<Fq: FqTrait> EllipticProduct<Fq> {
    ...
    fn split_theta_point(P: &ThetaPoint<Fq>) -> ((Fq, Fq), (Fq, Fq)) {
    ...
    /// Models of Kummer lines and Galois representation,
    /// Razvan Barbulescu, Damien Robert and Nicolas Sarkis
    fn null_point_to_montgomery_curve(O0: &(Fq, Fq)) -> Curve<Fq> {
        let (a, b) = O0;
    ...
    /// Models of Kummer lines and Galois representation,
    /// Razvan Barbulescu, Damien Robert and Nicolas Sarkis
    fn theta_point_to_montgomery_point(O0: &(Fq, Fq), P: &(Fq, Fq)) -> PointX<Fq> {
        let (a, b) = *O0;
        let (U, V) = *P;
    ...
    pub fn split_to_product(
        ...
        let (O1, O2) = Self::split_theta_point(&null_point);

        // Compute Montgomery curve from dimension one
        // null points
        let E3 = Self::null_point_to_montgomery_curve(&O1);
        let E4 = Self::null_point_to_montgomery_curve(&O2);
        let E3E4 = EllipticProduct::new(&E3, &E4);

        // Now compute points on E3 x E4
        for (i, P) in eval_points.iter().enumerate() {
            // Split to level 1
            let (P1, P2) = Self::split_theta_point(P);
            // Compute the XPoint (X : Z) from each theta point
            let Q1X = Self::theta_point_to_montgomery_point(&O1, &P1);
            let Q2X = Self::theta_point_to_montgomery_point(&O2, &P2);
