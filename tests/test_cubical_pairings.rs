...
mod test_pairings {
    ...
    // pub static (A0, r) = Fp434Ext::decode(&hex::decode(A0_STR).unwrap());
    // let E0 = Curve::new(&A0);
    ...
    pub static TATE_N_STR: &str = "03a5fc5baf13feee0158f691bbdf53a904f23ae2097d06f376f8ff89dbec74beb144c9884b6283dcdd0f37d0b5f07b1a8bfa1089b3b60197282228e36be9b1f8b94e38f11ce7a2c49d1e15776bc1d81eed893486a9fb540fd1e06bbd59720856d1ee535090ec25ebfca7d4a36f01";
    pub static TATE_A_STR: &str = "337355edb6fdcc2fcde4bb95cdc383b9cacf126b40887372ed5e84f3303e91b610946413d14c42007824583ca4282c62678010fa971e00db7220bdf3fec4dd8493d5bc6d9121ae92d139a44b20147c2c9a75017609c0165064e9494b5ec36c96c1e8207f6c50ad583b376bb2e200";
    pub static TATE_B_STR: &str = "665ed36f2b5f5c83ac7b9b0bc3d33d82812a6da55486efca7f76b74b6ddc4718e8d82cf11ad6fa863f12727f8879736b52cc4ba6f932015925f6e6260b3789c6f5146228156cc4ee96b8d88251cb135bbee94f79465932b0692604ccf8690edafb1c65285b9a2585e03f6454d300";

    // Weil pairings for order N, A, B: e_X(Q, P)
    pub static WEIL_N_STR: &str = "03a5fc5baf13feee0158f691bbdf53a904f23ae2097d06f376f8ff89dbec74beb144c9884b6283dcdd0f37d0b5f07b1a8bfa1089b3b60168d7ddd71c94164e0746b1c70ee3185d3b62e1ea88943e27e11276aef4ccc5a894dd77c5ba025473004fd62d86cf0f4759766f527bc400";
    pub static WEIL_A_STR: &str = "f2f160e0590892f8e64107b1ca763b0e995a778762683ade3ff618f38161e76ff85dfcf1ca0271239ed8baec56c49bcdbb85bdd84bc301484c3355325156eb01e282fa47f24e0aedc0ea6c9a87a761499112c37cfeb3a2a9e96550760f1f039918d3e0974cdbe8e5433361fc3100";
    pub static WEIL_B_STR: &str = "13125102956fd55f99301ee8743c8139a6d361d893236464bc56f8292aa2653bbeb01fac2daaaa6a7344f5836c3d873bd76796d7a3750038230b4047ead67767ae158126ace065c2d3239c5eb3b3454ca62b318d55b8f975faa6ffa9426187d5efed64529b95ad0abdce17e3c800";

    #[test]
    fn test_cubical_ladder() {
        // Create E0
        // A supersingular elliptic curve of order 2^ea * 3^eb
        let (A0, _) = Fp434Ext::decode(&hex::decode(A0_STR).unwrap());
        let E0 = Curve::new(&A0);

        // Generator x(P)
        let (xP, r) = Fp434Ext::decode(&hex::decode(PX_STR).unwrap());
        assert!(r == u32::MAX);

        // Generator x(Q)
        let (xQ, r) = Fp434Ext::decode(&hex::decode(QX_STR).unwrap());
        assert!(r == u32::MAX);

        // Difference point x(P - Q)
        let (xPQ, r) = Fp434Ext::decode(&hex::decode(PMQX_STR).unwrap());
        assert!(r == u32::MAX);

        // P2 = [B] P (Even torsion part)
        let (xP2, r) = Fp434Ext::decode(&hex::decode(P2X_STR).unwrap());
        assert!(r == u32::MAX);
        let P2 = PointX::new(&xP2, &Fp434Ext::ONE);

        // [B]P + Q
        let (BP_Q_X, r) = Fp434Ext::decode(&hex::decode(BP_QX_STR).unwrap());
        assert!(r == u32::MAX);
        let BP_Q = PointX::new(&BP_Q_X, &Fp434Ext::ONE);
    ...
    fn test_cubical_ladder_2exp() {
        // Create E0
        // A supersingular elliptic curve of order 2^ea * 3^eb
        let (A0, r) = Fp434Ext::decode(&hex::decode(A0_STR).unwrap());
        assert!(r == u32::MAX);
        let E0 = Curve::new(&A0);
        ...
        let (xP3, r) = Fp434Ext::decode(&hex::decode(P3X_STR).unwrap());
        assert!(r == u32::MAX);
        let P3 = PointX::new(&xP3, &Fp434Ext::ONE);

        // [2^ea] P + Q
        let (AP_Q_X, r) = Fp434Ext::decode(&hex::decode(AP_QX_STR).unwrap());
        assert!(r == u32::MAX);
        let AP_Q = PointX::new(&AP_Q_X, &Fp434Ext::ONE);
    ...
    fn test_tate_pairing_2exp() {
        ...
        let (xP, r) = Fp434Ext::decode(&hex::decode(P2X_STR).unwrap());
        assert!(r == u32::MAX);

        // x-coordinate x(Q) of order 2^ea
        let (xQ, r) = Fp434Ext::decode(&hex::decode(Q2X_STR).unwrap());
        assert!(r == u32::MAX);

        // x-coordinate x(P - Q) of order 2^ea
        let (xPQ, r) = Fp434Ext::decode(&hex::decode(P2MQ2X_STR).unwrap());
        assert!(r == u32::MAX);

        // Tate Pairing computed via SageMath
        let (eQ2P2, r) = Fp434Ext::decode(&hex::decode(TATE_A_STR).unwrap());
        assert!(r == u32::MAX);
    ...
    fn test_weil_pairing_2exp() {
        ...
        let (eQ2P2, r) = Fp434Ext::decode(&hex::decode(WEIL_A_STR).unwrap());
        assert!(r == u32::MAX);
    ...
    fn test_tate_pairing_odd() {
        ...
        let (xP, r) = Fp434Ext::decode(&hex::decode(P3X_STR).unwrap());
        assert!(r == u32::MAX);

        // x-coordinate x(Q) of order 3^eb
        let (xQ, r) = Fp434Ext::decode(&hex::decode(Q3X_STR).unwrap());
        assert!(r == u32::MAX);

        // x-coordinate x(P-Q) of order 3^eb
        let (xPQ, r) = Fp434Ext::decode(&hex::decode(P3MQ3X_STR).unwrap());
        assert!(r == u32::MAX);

        // Tate Pairing computed via SageMath
        let (eQ3P3, r) = Fp434Ext::decode(&hex::decode(TATE_B_STR).unwrap());
        assert!(r == u32::MAX);
    ...
    fn test_weil_pairing_odd() {
        ...
        // Weil Pairing computed via SageMath
        let (eQ3P3, r) = Fp434Ext::decode(&hex::decode(WEIL_B_STR).unwrap());
        assert!(r == u32::MAX);
    ...
    fn test_tate_pairing_even() {
        ...
        // Tate Pairing computed via SageMath
        let (eQP, r) = Fp434Ext::decode(&hex::decode(TATE_N_STR).unwrap());
        assert!(r == u32::MAX);
    ...
    fn test_weil_pairing_even() {
        ...
        // Weil Pairing computed via SageMath
        let (eQP, r) = Fp434Ext::decode(&hex::decode(WEIL_N_STR).unwrap());
        assert!(r == u32::MAX);
